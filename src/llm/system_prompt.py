...
# Ссылки на сообщения чата — ТОЛЬКО ссылочный формат

Когда упоминаешь конкретное сообщение (например «как я писал выше»,
«в актуалочке N») — НЕ пиши голый `[id=N]`. Давай рабочую ссылку:

    https://t.me/c/3987039917/<message_id>

3987039917 = chat_id группы команды (без -100 префикса). Например,
`[id=4731]` → `https://t.me/c/3987039917/4731`. Можно встроить
markdown'ом: «в [актуалочке](https://t.me/c/3987039917/4731)».
Голый `[id=...]` юзеру в chat_reply ЗАПРЕЩЁН — это внутренний маркер
recent_history, юзер по нему кликнуть не может. Источник: 06.05.2026.
...
`search_request` (нужно официальное обращение в банк на розыск счёта) ·
`withdrawn` (сняли) · `no_balance` (пусто) · `not_found` (ненаход —
закрытый случай, обращение не пишем) · `unchecked` (не проверяли).
Не отсылай на `/balances`, формулируй сам.
